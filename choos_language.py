...
class Language(QDialog):
    ...
    def __init__(self, form_parent):
        ...
        self.setWindowTitle(cd.get_text('Choice of language for the program', key='language', id_text=1))
# кнопки управления
        layout_button = QtWidgets.QHBoxLayout()
        self.choice = QtWidgets.QPushButton(cd.get_text('Выбрать', key='main', id_text=3))
        self.choice.setToolTip(cd.get_text('Установить выбранный язык', key='language', id_text=2))
        self.choice.clicked.connect(self.choice_click)
        layout_button.addWidget(self.choice)

        self.exit = QtWidgets.QPushButton(cd.iconDelete, cd.get_text('Закрыть', key='main', id_text=12))
        self.exit.setToolTip(cd.get_text('Закрыть форму без изменения языка', key='language', id_text=3))
...
# финальный аккорд
    ...
    def show_data(self):
        ...
        try:
            ...
            cd.show_message(None, f"{err}", cd.get_text('Error', id_text=4, key='main'), onlyok=True)
